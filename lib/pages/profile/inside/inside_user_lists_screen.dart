...
class InsideUserListsScreen extends StatefulWidget {
  final String uid;
  final String username;
  final int? count;
  const InsideUserListsScreen(
      {super.key, required this.uid, required this.username, this.count});

  @override
  State<InsideUserListsScreen> createState() => _InsideUserListsScreenState();
}

class _InsideUserListsScreenState extends State<InsideUserListsScreen> {
    ...
    return Scaffold(
      ...
      body: _isLoading
          ? SingleChildScrollView(
              child: Column(
                children: [
                  if (widget.count != null && widget.count! > 10)
                    for (int i = 0; i < 10; i++) const LoadingUserBoxShimmer()
                  else if (widget.count != null && widget.count! < 10)
